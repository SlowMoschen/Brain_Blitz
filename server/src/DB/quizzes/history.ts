//MARK: Der 2. Weltkrieg
const ww2Quiz = {
	title: 'Der 2. Weltkrieg',
	description: 'Geschenisse über den Zweiten Weltkrieg',
	category: 'Geschichte',
	questions: [
		{
			question: 'Welches Ereignis löste den Ausbruch des Zweiten Weltkriegs aus?',
			answers: [
				'Der Angriff auf Pearl Harbor',
				'Die Invasion Polens durch Deutschland',
				'Die Bombardierung von London',
				'Die Kapitulation Frankreichs',
			],
			correct_answer: 'Die Invasion Polens durch Deutschland',
		},
		{
			question: 'Welche Länder waren die Hauptachsenmächte im Zweiten Weltkrieg?',
			answers: [
				'Deutschland, Italien und Japan',
				'Großbritannien, Frankreich und USA',
				'Russland, China und Australien',
				'Spanien, Portugal und Kanada',
			],
			correct_answer: 'Deutschland, Italien und Japan',
		},
		{
			question: 'Welches Jahr begann der Zweite Weltkrieg?',
			answers: ['1937', '1939', '1941', '1943'],
			correct_answer: '1939',
		},
		{
			question: 'Welches Ereignis führte zum Eintritt der USA in den Zweiten Weltkrieg?',
			answers: [
				'Die Bombardierung von Pearl Harbor',
				'Die Schlacht um Stalingrad',
				'Die Invasion von Normandie',
				'Die Bombardierung von Hiroshima',
			],
			correct_answer: 'Die Bombardierung von Pearl Harbor',
		},
		{
			question: 'Welches Land gab als erstes den Einsatz der Atombombe bekannt?',
			answers: ['Deutschland', 'Japan', 'Italien', 'USA'],
			correct_answer: 'Japan',
		},
		{
			question: 'Welche Länder waren die Alliierten im Zweiten Weltkrieg?',
			answers: [
				'Deutschland, Italien und Japan',
				'Großbritannien, Frankreich und Russland',
				'USA, Großbritannien und die Sowjetunion',
				'Japan, China und Australien',
			],
			correct_answer: 'USA, Großbritannien und die Sowjetunion',
		},
		{
			question: 'Welches Land kapitulierte zuerst im Zweiten Weltkrieg?',
			answers: ['Deutschland', 'Japan', 'Italien', 'Frankreich'],
			correct_answer: 'Frankreich',
		},
		{
			question: 'Welches war das letzte Land, das Deutschland im Zweiten Weltkrieg angegriffen hat?',
			answers: ['Großbritannien', 'Sowjetunion', 'USA', 'Frankreich'],
			correct_answer: 'Sowjetunion',
		},
		{
			question: 'Welches Land verlor am meisten Menschen im Zweiten Weltkrieg?',
			answers: ['USA', 'Großbritannien', 'Sowjetunion', 'Japan'],
			correct_answer: 'Sowjetunion',
		},
		{
			question: 'Welches Jahr endete der Zweite Weltkrieg?',
			answers: ['1943', '1945', '1947', '1950'],
			correct_answer: '1945',
		},
	],
};

// MARK: Ereiignisse die die Welt veränderten
const historyEventsQuiz = {
	title: 'Ereignisse die die Welt veränderten',
	description: 'Wichtige Ereignisse in der Geschichte der Menschheit',
	category: 'Geschichte',
	questions: [
		{
			question: 'Welches Ereignis führte zum Ausbruch des Ersten Weltkriegs?',
			answers: [
				'Die Ermordung von Erzherzog Franz Ferdinand',
				'Die Bombardierung von Pearl Harbor',
				'Die Invasion Polens durch Deutschland',
				'Die Kapitulation Frankreichs',
			],
			correct_answer: 'Die Ermordung von Erzherzog Franz Ferdinand',
		},
		{
			question: 'Welche Auswirkungen hatte die erfindung des Internets auf die Welt?',
			answers: [
				'Erleichterung der Kommunikation und des Informationsaustauschs',
				'Verbesserung der medizinischen Versorgung',
				'Erhöhung der Lebenserwartung',
				'Verbesserung der Bildung',
			],
			correct_answer: 'Erleichterung der Kommunikation und des Informationsaustauschs',
		},
		{
			question: 'Wann wurde die Unabhängigkeit der USA erklärt?',
			answers: ['1776', '1789', '1800', '1812'],
			correct_answer: '1776',
		},
		{
			question: '1969 landete der erste Mensch auf dem Mond. Wie hieß dieser Astronaut?',
			answers: ['Neil Armstrong', 'Buzz Aldrin', 'Yuri Gagarin', 'John Glenn'],
			correct_answer: 'Neil Armstrong',
		},
		{
			question: 'Von wann bis wann stand die Berliner Mauer?',
			answers: ['1961-1989', '1945-1990', '1939-1945', '1914-1918'],
			correct_answer: '1961-1989',
		},
		{
			question: '1945 wurde durch das Projekt Manhattan die erste Atombombe entwickelt. Wo wurde sie getestet und wie hieß der Test?',
			answers: ['New Mexico, Trinity', 'Hawaii, Aloha', 'Nevada, Silver', 'Arizona, Phoenix'],
			correct_answer: 'New Mexico, Trinity',
		},
		{
			question: 'Wann wurde die Sklaverei in den USA offiziell abgeschafft?',
			answers: ['1865', '1776', '1800', '1812'],
			correct_answer: '1865',
		},
		{
			question: 'Wofür ist Kriegskommandant Hannibal bekannt?',
			answers: ['Elefanten über die Alpen geführt', 'Die Schlacht von Waterloo gewonnen', 'Die Römer besiegt', 'Die Chinesische Mauer gebaut'],
			correct_answer: 'Elefanten über die Alpen geführt',
		},
		{
			question: 'Im März 2022 wurde die Invasion welches Landes durch Russland gestartet?',
			answers: ['Ukraine', 'Georgien', 'Estland', 'Weißrussland'],
			correct_answer: 'Ukraine',
		},
		{
			question: 'Was war die Ursache für den Ausbruch des Dreißigjährigen Krieges?',
			answers: ['Prager Fenstersturz', 'Französische Revolution', 'Kolonialismus', 'Kalter Krieg'],
			correct_answer: 'Prager Fenstersturz',
		}
	]
}

//MARK: Persönlichkeiten der Geschichte
const persoenlichkeitenQuiz = {
	title: 'Persönlichkeiten der Geschichte',
	description: 'Fragen über berühmte Persönlichkeiten',
	category: 'Geschichte',
	questions: [
		{
			question: 'Wer war der erste Präsident der Vereinigten Staaten von Amerika?',
			answers: ['Thomas Jefferson', 'George Washington', 'Abraham Lincoln', 'John Adams'],
			correct_answer: 'George Washington',
		},
		{
			question: "Welcher Komponist gilt als 'Vater der Klassik' und komponierte die 'Ode an die Freude'?",
			answers: ['Wolfgang Amadeus Mozart', 'Ludwig van Beethoven', 'Johann Sebastian Bach', 'Franz Schubert'],
			correct_answer: 'Ludwig van Beethoven',
		},
		{
			question: 'Welcher Künstler malte die Mona Lisa?',
			answers: ['Michelangelo', 'Vincent van Gogh', 'Leonardo da Vinci', 'Pablo Picasso'],
			correct_answer: 'Leonardo da Vinci',
		},
		{
			question: 'Wer war die erste Frau, die den Nobelpreis erhielt?',
			answers: ['Marie Curie', 'Amelia Earhart', 'Florence Nightingale', 'Jane Addams'],
			correct_answer: 'Marie Curie',
		},
		{
			question: 'Welcher Forscher entdeckte die Schwerkraft, als ihm ein Apfel auf den Kopf fiel?',
			answers: ['Galileo Galilei', 'Isaac Newton', 'Albert Einstein', 'Charles Darwin'],
			correct_answer: 'Isaac Newton',
		},
		{
			question:
				"Wer war der Anführer der amerikanischen Bürgerrechtsbewegung und hielt die berühmte Rede 'I Have a Dream'?",
			answers: ['Malcolm X', 'Nelson Mandela', 'Martin Luther King Jr.', 'Mahatma Gandhi'],
			correct_answer: 'Martin Luther King Jr.',
		},
		{
			question:
				"Welcher römische Herrscher war berühmt für seinen Ausspruch 'Alea iacta est' (Die Würfel sind gefallen)?",
			answers: ['Augustus', 'Julius Caesar', 'Marcus Aurelius', 'Caligula'],
			correct_answer: 'Julius Caesar',
		},
		{
			question:
				'Welcher Politiker wurde 1940 zum Premierminister des Vereinigten Königreichs gewählt und führte sein Land während des Zweiten Weltkriegs?',
			answers: ['Winston Churchill', 'Franklin D. Roosevelt', 'Joseph Stalin', 'Charles de Gaulle'],
			correct_answer: 'Winston Churchill',
		},
		{
			question:
				'Welcher berühmte französische Kaiser führte seine Truppen bei der Schlacht von Wien im Jahr 1809 an, die seine erste bedeutende Niederlage markierte?',
			answers: ['Napoleon Bonaparte', 'Karl V.', 'Ludwig XIV.', 'Alexander I.'],
			correct_answer: 'Napoleon Bonaparte',
		},
		{
			question: 'Wer war der Gründer des Islam?',
			answers: ['Mohammed', 'Jesus Christus', 'Moses', 'Buddha'],
			correct_answer: 'Mohammed',
		},
	],
};

export const historyQuizzes = [
	ww2Quiz,
	persoenlichkeitenQuiz,
	historyEventsQuiz
];
